...
> drawMazeStr :: Maze -> String
> drawMazeStr maze = concat [drawMazeEW maze y ++ "\n" ++ drawMazeNS maze y  ++ "\n" | y <- reverse[0..height]]
>	where (width, height) = sizeOf maze

> drawMazeEW :: Maze -> Int -> String
> drawMazeEW maze y = drawMazeEW1 maze 0 y

> drawMazeEW1 :: Maze -> Int -> Int -> String
> drawMazeEW1 maze x y
>	| y == height        = ""
>	| x == width         = "|"
>	| hasWall maze (x,y) W = "|   " ++ (drawMazeEW1 maze (x+1) y)
>	| otherwise            = "    " ++ (drawMazeEW1 maze (x+1) y)
>		where (width,height) = sizeOf maze

> drawMazeNS :: Maze -> Int -> String
> drawMazeNS maze y = drawMazeNS1 maze 0 y

> drawMazeNS1 :: Maze -> Int -> Int -> String 
> drawMazeNS1 maze x y 
>	| x == width         = "+"
>	| hasWall maze (x,y) S = "+---" ++ (drawMazeNS1 maze (x+1) y)
...
> solveMazeIter :: Maze -> Place -> [(Place, Path)] -> Maybe Path
> solveMazeIter maze target [] = Nothing
> solveMazeIter maze target ((xy, directions):rest)
>       | xy == target = Just (reverse (directions))
>       | True = solveMazeIter maze target (rest ++ [(move d xy, d:directions) | d <- ds])
>           where ds = filter (\d -> (not (hasWall maze xy d))) [N,E,S,W]

> fastSolveMaze :: Maze -> Place -> Place -> Maybe Path
...
> fastSolveMazeIter maze target [] visited = Nothing
> fastSolveMazeIter maze target ((xy, directions):rest) visited
>       | xy == target = Just (reverse (directions))
>       | otherwise = fastSolveMazeIter maze target (rest ++ [(move d xy, d:directions) | d <- notVisited]) (xy:visited)
>           where notWalls = filter (\d -> (not (hasWall maze xy d))) [N,E,S,W]
>                 notVisited = filter (\d -> not (elem (move d xy) visited)) notWalls
